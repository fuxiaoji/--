...
#### 左下：效用函数 U(d₂) — 注意全局最大 ≠ 可行最优

- **淡紫线**：不可行域（$d_2 < 17.76$），超出安全约束
- **深紫线**：安全可行域（$d_2 \ge 17.76$）
- **灰色圆点**：**无约束全局最大值**（$d_2 \approx 11.5$ mm，$U=0.385$）—— 但在这个厚度下，>44°C 时间约 2000s，是安全限值 300s 的 **6 倍以上**，严重违反约束
- **绿色圆点**：**约束最优解**（$d_2 = 17.76$ mm，$U=0.337$）—— 安全可行域内的最大值
- **红色半透明区域**：不可行域（$d_2$ 太小 → 超温）

**关键观察**：如果不考虑安全约束，U 的全局最大值在 d₂=11.5mm 附近——这个厚度既能保持一定舒适度（C≈0.33），又有不错的隔热（R≈0.45）。但 11.5mm 的服装在 65°C 环境下会导致皮肤超温 2000+ 秒，完全不安全。

**安全的代价**：约束使最优效用从 0.385 降到 0.337，损失约 **12%**。这个数字量化了"安全约束对设计自由的压缩"。绿点在整条紫线下坡段的中段，不是曲线的波峰——波峰被安全约束切掉了。
